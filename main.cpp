...
const string LEADER_PATH = "img/minimalistic-plane/leader-in-area.png";
const string ENEMY_PATH = "img/minimalistic-plane/enemy-in-area.png";
const string LEADER_ENTITY_PATH = "img/minimalistic-plane/leader-entity.png";
const string ENEMY_ENTITY_PATH = "img/minimalistic-plane/enemy-entity.png";
const string BULLET_PATH  = "img/minimalistic-plane/bullet.png";
const string BLACKHOLD_PATH = "img/blackhole.jpg";
...
Vector2 gravity_position(800, 0);

Gravity gr(gravity_position, 1);
    ...
    if (!init()) {
        ...
        if (!loadMedia()) {
            ...
            while (!done) {
                ...
                ticks = currentTime - previousFrameTime;
                ...
                if (!gameOver) {
                    ...
                    gr.render(gr.position.x, gr.position.y, NULL, renderer, 100, 100);

                    leader1->updatePosition(ticks);
                    leader2->updatePosition(ticks);

                    gr.update(ticks);
                    gr.update(leader1, ticks);
                    gr.update(leader2, ticks);

                    // Draw the first player
                    leader1->render(leader1->position.x, leader1->position.y, NULL, renderer, 50, 50);
                    leader1->updateCenter();
//                    draw_rect((int)leader1->center.x, (int)leader1->center.y, 5, 5);
                    
                    
                    // Draw the second player
                    leader2->render(leader2->position.x, leader2->position.y, NULL, renderer, 50, 50);
...
//                    draw_rect((int)leader2->center.x, (int)leader2->center.y, 5, 5);
                    ...
                    for (Entity* e : leader1Entities) {
                        e->updateCenter();
                        e->render(e->position.x, e->position.y, NULL, renderer, 30, 30);
                    ...
                    if (KEYS[SDL_SCANCODE_UP]) {
                        leader1->updateSpeed(1);
                    }else if (KEYS[SDL_SCANCODE_DOWN]) {
                        leader1->updateSpeed(-1);
                    }else{
                        leader1->speedDown(ticks);
                    }
...
bool loadMedia()
{
    ...
    if (!leader2->loadFromFile(ENEMY_PATH, renderer)) {
        IMGloaded = false;
    }gr.loadFromFile(BLACKHOLD_PATH, renderer);
